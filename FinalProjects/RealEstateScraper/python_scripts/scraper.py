...
class PropertyImageScraper:
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                if attempt < retry_count:
                    ...
                    self.logger.warning(f"Attempt {attempt + 1}/{retry_count + 1} failed for {url}: {str(e)}. Retrying in {wait_time}s...")
    ...
    def extract_images(self, soup, base_url):
        ...
        gallery_containers = soup.select('.re__pr-carousel-wrapper, .image-gallery, .property-images, .carousel-inner, .slider-wrapper')
        ...
        if not image_urls:
            ...
            content_images = soup.select('.re__pr-carousel-item img, .content-area img, .property-detail img, .main-content img')
        ...
        try:
            ...
            for i, img_url in enumerate(image_urls):
                try:
                    ...
                    saved = save_image(img_url, img_path, self.headers, self.timeout, self.retry_count)
                    ...
                    self.logger.warning(f"Failed to save image {img_url}: {str(e)}")
